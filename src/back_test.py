...
def kupiec_uc_test(exceed: np.ndarray, alpha: float) -> dict:
    ...
    lr = -2 * (
        (n - x) * np.log(1 - a) + x * np.log(a)
        - ((n - x) * np.log(1 - p) + x * np.log(p))
    )
    pval = 1 - chi2.cdf(lr, df=1)
    return {"n": n, "exceptions": x, "exception_rate": phat, "LR_uc": float(lr), "p_value": float(pval)}


def backtest_var(df_results: pd.DataFrame, var_col: str, alpha: float = 0.01) -> pd.DataFrame:
...
def backtest_es_simple(df_results: pd.DataFrame, es_col: str, alpha_es: float = 0.025) -> pd.DataFrame:
    """
    Backtest ES simple:
    ...
    return pd.DataFrame([{
        ...
        "mean_loss_worst_(top_2.5%)": float(worst.mean()) if len(worst) else np.nan,
